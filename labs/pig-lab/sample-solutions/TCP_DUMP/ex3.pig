A = LOAD '/laboratory/tstat-big.txt' using PigStorage(' ') AS (ip_c:chararray,
port_c:int,
packets_c:int,
rst_c:int,
ack_c:int,
...
B= GROUP A BY ip_c;
C= FOREACH B GENERATE group, SUM(A.data_bytes_c) AS transmitted, SUM(A.data_bytes_s) AS received, SUM(A.data_bytes_s)+SUM(A.data_bytes_c) AS total;

-- TODO: Store output data in ./local-output/TCP_DUMP/
STORE C INTO '/user/group001/pig/TCP_DUMP/ex3';
